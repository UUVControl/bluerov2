...
dt = 0.01;
  ...
  % Hydrostatics Matrix
    ...
    %Velocity Matrix
%     vel = (M)\(-Dv)+(M)\diag(-g);
    vel = [(-sum(Dv(1,:))-sum(g(1,:)))/sum(M(1,:));(-sum(Dv(2,:))-sum(g(2,:)))/sum(M(2,:));(-sum(Dv(3,:))-sum(g(3,:)))/sum(M(3,:));(-sum(Dv(4,:))-sum(g(4,:)))/sum(M(4,:));(-sum(Dv(5,:))-sum(g(5,:)))/sum(M(5,:));(-sum(Dv(6,:))-sum(g(6,:)))/sum(M(6,:))];
    
    %state transition using first order explicit Runge Kunta Method
    x = x +[sum(pos(1,:));sum(pos(2,:));sum(pos(3,:));sum(pos(4,:));sum(pos(5,:));sum(pos(6,:));vel(1,:)+u(1,:);vel(2,:)+u(2,:);vel(3,:)+u(3,:);vel(4,:)+u(4,:);vel(5,:)+u(5,:);vel(6,:)+u(6,:)]*dt;
end
